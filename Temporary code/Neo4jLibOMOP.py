...
def OneTimeCode():
    ...
    CypherBoltQuery("match (n:Provider) delete n",database="omop")
    ...
    CreateIndex("Visit_Occurrence","visit_occurrence_id",db='omop')
    CreateIndex("Visit_Occurrence","person_id_id",db='omop')
    CreateIndex("Visit_Occurrence","provider_id",db='omop')
    CreateIndex("Visit_Occurrence","care_site_id",db='omop')

    cq="USING PERIODIC COMMIT 10000 LOAD CSV WITH HEADERS FROM 'file:///omop/VisitSequence.csv' as line FIELDTERMINATOR '|' match (v1:Visit_Occurrence{person_id:toInteger(line.person_id),visit_occurrence_id:toInteger(line.fromNode)})  match (v2:Visit_Occurrence{person_id:toInteger(line.person_id),visit_occurrence_id:toInteger(line.toNode)}) merge (v1)-[r:visit_sequence]->(v2)"
    UploadWithPeriodicCommit(cq,"omop")
